...
export default function AllHistoryPage() {
  const history_habit_card = require("../Assests/history_habit_card1.jpg");
  const {id} = useParams()
  const data: any  = useSelector((state: RootState) => state.updateHabit);
  const {updateHabit } = useSelector((state: RootState) => state);

  return (
    // This page Display All the habits stored and clicking on that habit it will  shows the particular habit status days wise justify-end items-center
    <div className="home-page flex  px-3 md:pr-[20%] ">
